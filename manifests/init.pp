...
)
{
  ...
  file
  { "/usr/local/exist-distribution-${exist_version}" :
    ensure  => directory,
    owner   => $exist_user,
    group   => $exist_group,
    recurse => true,
  ...
  if $exist_service == 'eXist-db'
  {
    file
    { "/etc/systemd/system/${exist_service}.service" :
      ensure => file,
      owner  => 'root',
      group  => 'root',
      mode   => '0644',
      source => "puppet:///modules/existdb/${exist_service}.service",
  ...
  else
  {
    ...
    { "/etc/systemd/system/${exist_service}.service" :
      ...
      group  => 'root',
      mode   => '0644',
      source => "puppet:///modules/profiles/etc/systemd/system/${exist_service}.service",
      notify => Service[$exist_service],
    }
  }
